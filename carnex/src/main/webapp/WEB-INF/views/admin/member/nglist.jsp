...
<script>
	...
	<section>
		<div class="tab-content">
		    ...
		    <div class="content-wrap ngmember-list">
		    	<h3>회원 검색</h3>
				...
				<ul class="mb-info-wrap">
					...
					<c:if test="${empty ngList}">
					<li>
						<p> ※ 비회원이 존재하지 않습니다.</p>
					</li>
					</c:if>
					<c:if test="${not empty ngList}">
						<c:forEach items="${ngList}" var="mvo">
						<li>
							<p>${mvo.user_id}</p>
							<p><a href="/admin/member/memberinfo?user_id=${mvo.user_id}">${mvo.user_nick}</a></p>
							<p>${mvo.user_name}</p>
